---
layout: default
title: Gunships32
parent: Participants
nav_order: 10
nav_exclude: true
search_exclude: true
published: false
participant_image: "/assets/participants/Gunships32.png"
---

<div class="nhc-participant-header">
  <img
    class="nhc-participant-avatar"
    src="{{ '/assets/participants/Gunships32.png' | relative_url }}"
    alt="Avatar de Gunships32"
  >

  <div>
    <h1 class="nhc-participant-name">Gunships32</h1>

    <p class="nhc-participant-summary">
      Participant de la Nuclear Haunted Cup #3 : Apocalypse.
    </p>

    <div class="nhc-participant-links">
      <a
        class="nhc-action"
        href="https://www.twitch.tv/gunships32"
        target="_blank"
        rel="noopener noreferrer"
      >
        Twitch
      </a>
    </div>
  </div>
</div>

## Présentation

À venir.

## Informations

**Expérience sur Phasmophobia :** À venir<br>
**Spécialité :** À venir

## À propos

À venir.

## Parcours dans la Nuclear Haunted Cup

Première participation.
